...
from modules.cash_float_builder.core.float import build_float
...
app = FastAPI(title="Cash Float Builder")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("cash_float_builder", base_url=FLOW_BASE_URL)
...
def build(
    ...
    counts: str | None = Form(None),
    target: str | None = Form(None),
    decimals: str | None = Form("2"),
):
    result, error = build_float(
        denominations,
        counts,
        target=target,
        decimals=decimals,
