...
from modules.flashcard_builder.core.generate import build_flashcards
...
app = FastAPI(title="Flashcard Builder")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("flashcard_builder", base_url=FLOW_BASE_URL)
...
@app.post("/generate")
async def generate(file: UploadFile | None = File(None), text: str | None = Form(None), limit: int = Form(8)):
    resolved, error = await _read_text(file, text)
    if error:
        return JSONResponse({"error": error}, status_code=400)

    payload, error = build_flashcards(resolved, limit)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
