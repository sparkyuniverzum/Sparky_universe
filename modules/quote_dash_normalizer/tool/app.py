...
from modules.quote_dash_normalizer.core.normalize import normalize_quotes_dashes
...
app = FastAPI(title="Quote & Dash Normalizer")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("quote_dash_normalizer", base_url=FLOW_BASE_URL)
...
async def normalize(
    ...
    payload, error = normalize_quotes_dashes(resolved, mode=mode)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
