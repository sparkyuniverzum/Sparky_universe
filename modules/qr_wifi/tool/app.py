...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("qr_wifi", base_url=FLOW_BASE_URL)
...
@app.post("/build")
def build(
    ssid: str | None = Form(None),
    password: str | None = Form(None),
    security: str | None = Form("WPA"),
    hidden: bool = Form(False),
):
    payload, error = build_wifi_payload(ssid, password, security, hidden)
    if error:
