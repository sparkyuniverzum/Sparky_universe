...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("smart_truncate", base_url=FLOW_BASE_URL)
...
async def truncate(
    ...
    limit: int = Form(200),
    unit: str = Form("chars"),
    add_ellipsis: str | None = Form("on"),
    prefer_sentence: str | None = Form("on"),
):
    resolved, error = await _read_text(file, text)
    if error:
        return JSONResponse({"error": error}, status_code=400)

    payload, error = smart_truncate(
        resolved,
        limit=limit,
        unit=unit,
        add_ellipsis=bool(add_ellipsis),
        prefer_sentence=bool(prefer_sentence),
    )
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
