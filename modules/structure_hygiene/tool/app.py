...
from modules.structure_hygiene.core.check import structure_hygiene
...
app = FastAPI(title="Structure Hygiene")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("structure_hygiene", base_url=FLOW_BASE_URL)
...
async def check(
    ...
    payload, error = structure_hygiene(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
