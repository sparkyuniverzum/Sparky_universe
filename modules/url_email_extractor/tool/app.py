...
from modules.url_email_extractor.core.extract import extract_urls_emails
from universe.ads import attach_ads_globals
from universe.flows import resolve_flow_links
from universe.settings import shared_templates_dir

app = FastAPI(title="URL & Email Extractor")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("url_email_extractor", base_url=FLOW_BASE_URL)
...
async def extract(
    ...
    payload, error = extract_urls_emails(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
