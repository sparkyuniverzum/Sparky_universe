...
from modules.invisible_whitespace_map.core.map import map_whitespace
...
app = FastAPI(title="Invisible Whitespace Map")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("invisible_whitespace_map", base_url=FLOW_BASE_URL)
...
async def map_view(
    ...
    payload, error = map_whitespace(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
