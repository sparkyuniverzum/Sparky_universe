...
from modules.numbers_units_consistency.core.check import numbers_units_consistency
...
app = FastAPI(title="Numbers and Units Consistency")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("numbers_units_consistency", base_url=FLOW_BASE_URL)
...
async def check(
    ...
    payload, error = numbers_units_consistency(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
