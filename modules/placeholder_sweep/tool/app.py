...
from modules.placeholder_sweep.core.check import placeholder_sweep
...
app = FastAPI(title="Placeholder and TODO Sweep")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("placeholder_sweep", base_url=FLOW_BASE_URL)
...
async def check(
    ...
    payload, error = placeholder_sweep(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
