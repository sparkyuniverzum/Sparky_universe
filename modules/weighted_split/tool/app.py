...
from modules.weighted_split.core.split import split_weighted
...
app = FastAPI(title="Weighted Split")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("weighted_split", base_url=FLOW_BASE_URL)
...
def split(
    ...
    mode: str | None = Form("weights"),
    decimals: str | None = Form("2"),
):
    result, error = split_weighted(amount, weights, mode=mode, decimals=decimals)
    if error:
