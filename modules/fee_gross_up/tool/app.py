...
from modules.fee_gross_up.core.fees import compute_fee
...
app = FastAPI(title="Fee Gross Up")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("fee_gross_up", base_url=FLOW_BASE_URL)
...
def compute(
    ...
    percent_fee: str | None = Form("2.9"),
    fixed_fee: str | None = Form("0.30"),
    mode: str | None = Form("gross_from_net"),
    decimals: str | None = Form("2"),
):
    result, error = compute_fee(
        amount,
        percent_fee,
        fixed_fee,
        mode=mode,
