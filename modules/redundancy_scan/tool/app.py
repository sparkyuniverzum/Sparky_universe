...
from modules.redundancy_scan.core.check import redundancy_scan
...
app = FastAPI(title="Redundancy Scan")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("redundancy_scan", base_url=FLOW_BASE_URL)
...
async def check(
    ...
    payload, error = redundancy_scan(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
