...
from modules.cta_presence_check.core.check import cta_presence_check
...
app = FastAPI(title="CTA Presence Check")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("cta_presence_check", base_url=FLOW_BASE_URL)
...
async def check(
    ...
    payload, error = cta_presence_check(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
