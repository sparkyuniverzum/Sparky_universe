...
from modules.ambiguity_finder.core.check import ambiguity_finder
...
app = FastAPI(title="Ambiguity Finder")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("ambiguity_finder", base_url=FLOW_BASE_URL)
...
async def check(
    ...
    payload, error = ambiguity_finder(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
