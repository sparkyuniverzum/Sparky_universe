...
from modules.utmcheck.core.check import check_utms
...
app = FastAPI(title="UTM Link Checker")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("utmcheck", base_url=FLOW_BASE_URL)
...
@app.post("/check")
def check(urls: str | None = Form(None)):
    result, error = check_utms(urls)
    if error:
