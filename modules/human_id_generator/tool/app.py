...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("human_id_generator", base_url=FLOW_BASE_URL)
...
def generate(
    ...
    start: int = Form(1),
    width: int = Form(6),
    date_format: str = Form("YYYY-MM"),
    custom_date: str | None = Form(None),
    separator: str = Form("-"),
):
    payload, error = generate_ids(
        prefix=prefix,
        count=count,
        start=start,
        width=width,
