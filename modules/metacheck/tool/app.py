...
from modules.metacheck.core.check import audit_html
...
app = FastAPI(title="Meta Tag Checker")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("metacheck", base_url=FLOW_BASE_URL)
...
@app.post("/check")
def check(html: str | None = Form(None)):
    result, error = audit_html(html)
    if error:
