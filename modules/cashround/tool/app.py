...
from modules.cashround.core.rounding import calculate_cash_rounding
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("cashround", base_url=FLOW_BASE_URL)
...
def calc_rounding(
    ...
    result, error = calculate_cash_rounding(
        amount, step, mode=mode, paid=paid, decimals=decimals
    )
    if error:
