...
from modules.one_sentence_summary.core.generate import one_sentence_summary
...
app = FastAPI(title="One-Sentence Summary")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("one_sentence_summary", base_url=FLOW_BASE_URL)
...
async def generate(file: UploadFile | None = File(None), text: str | None = Form(None)):
    ...
    payload, error = one_sentence_summary(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
