...
from modules.terminology_consistency.core.check import terminology_consistency
from universe.ads import attach_ads_globals
from universe.flows import resolve_flow_links
from universe.settings import shared_templates_dir

app = FastAPI(title="Terminology Consistency")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("terminology_consistency", base_url=FLOW_BASE_URL)
...
async def check(
    ...
    payload, error = terminology_consistency(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
