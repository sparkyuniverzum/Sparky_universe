...
from modules.concept_map_outline.core.generate import build_concept_map
...
app = FastAPI(title="Concept Map Outline")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("concept_map_outline", base_url=FLOW_BASE_URL)
...
@app.post("/generate")
async def generate(file: UploadFile | None = File(None), text: str | None = Form(None), limit: int = Form(6)):
    resolved, error = await _read_text(file, text)
    if error:
        return JSONResponse({"error": error}, status_code=400)

    payload, error = build_concept_map(resolved, limit)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
