...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("study_plan_stub", base_url=FLOW_BASE_URL)
...
async def generate(file: UploadFile | None = File(None), text: str | None = Form(None)):
    ...
    payload, error = build_study_plan(resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
