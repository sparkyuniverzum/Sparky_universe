...
def make_change(
    ...
    *,
    ...
    total_dec, error = _parse_decimal(total, label="Total")
    if error or total_dec is None:
        return None, error
    ...
    if total_dec < Decimal("0") or paid_dec < Decimal("0"):
        return None, "Amounts must be non-negative."

    total_units, error = _to_minor_units(total_dec, decimals_int)
    if error:
        return None, error
    paid_units, error = _to_minor_units(paid_dec, decimals_int)
    if error:
        return None, error

    if paid_units < total_units:
        return None, "Paid amount must cover the total."

    denoms_units, error = _parse_denoms(denoms, decimals_int)
    if error or denoms_units is None:
        return None, error

    change_units = paid_units - total_units
    remainder = change_units
    ...
    return {
        "total": _quantize(total_dec, decimals_int),
        "paid": _quantize(paid_dec, decimals_int),
        "change": _quantize(
            Decimal(change_units) / (Decimal(10) ** decimals_int), decimals_int
        ),
        "breakdown": breakdown,
        "remainder": _quantize(
