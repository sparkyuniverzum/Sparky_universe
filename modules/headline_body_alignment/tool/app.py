...
from modules.headline_body_alignment.core.check import headline_body_alignment
from universe.ads import attach_ads_globals
from universe.flows import resolve_flow_links
from universe.settings import shared_templates_dir

app = FastAPI(title="Headline-Body Alignment")
...
def index(request: Request):
    ...
    flow_links = resolve_flow_links("headline_body_alignment", base_url=FLOW_BASE_URL)
...
async def check(
    ...
    payload, error = headline_body_alignment(headline, resolved)
    if error:
        return JSONResponse({"error": error}, status_code=400)
    return payload
